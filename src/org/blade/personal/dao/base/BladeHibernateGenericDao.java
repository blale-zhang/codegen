...
//import com.achievo.core.dao.support.Page;
//import com.achievo.core.utils.BeanUtils;
...
public class BladeHibernateGenericDao<T>{
	 ...
	 */
	public <T> Criteria createCriteria(Class<T> yClasentityClasss, Criterion... criterions) {
		Criteria criteria =this.sessionFactory.getCurrentSession().createCriteria(yClasentityClasss);
	...
	public <T> List<T> find(Class<T> entityClass,String hql, Object... values) {
		Assert.hasText(hql);
		return (List<T>) this.find(hql, values);
	}
	 ...
	 */
	public Pager<T> pagedQuery(String hql, Long pageNo, Long pageSize, Object... values) {
		Assert.hasText(hql);
		Assert.isTrue(pageNo >= 1, "pageNo should start from 1");
		Pager<T> page = new Pager<T>();
		// Count查询
		String countQueryString = " select count (*) " + removeSelect(removeOrders(hql));
		Query query = this.sessionFactory.getCurrentSession().createQuery(countQueryString);
		Object totalCountObject = query.uniqueResult();
		long totalCount = (Long) totalCountObject;
		...
		// 实际查询返回分页对象
		Long startIndex = page.getIndex();
		query = createQuery(hql, values);
		List list = query.setFirstResult(startIndex.intValue()).setMaxResults(pageSize.intValue()).list();
	 ...
	 */
	public Pager pagedQuery(Criteria criteria, Long pageNo, Long pageSize) {
		...
		// 实际查询返回分页对象
		Long startIndex = page.getIndex();
		List list = criteria.setFirstResult(startIndex.intValue()).setMaxResults(pageSize.intValue()).list();
	 ...
	 * @return 含总记录数和当前页数据的Page对象.
	 */
	public Pager pagedQuery(Class entityClass, Long pageNo, Long pageSize, Criterion... criterions) {
		Criteria criteria = createCriteria(entityClass, criterions);
	 ...
	 * @return 含总记录数和当前页数据的Page对象.
	 */
	public Pager pagedQuery(Class entityClass, Long pageNo, Long pageSize, String orderBy, boolean isAsc,
