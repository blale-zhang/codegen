 ...
 * usr-authority.js
 */
define('usr-authority',['jquery','base','liger.all','authority'],function(require, exports, module){
	...
	function initUrlList(){
		...
		var urlList = $("#urlList").ligerGrid({
	        ...
	        toolbar:{ items: [ { text: '激活菜单', click: attachUrlToUsr , icon:'add'}]  }
	    });
		
		/**
		 * 追加权限给指定用户
		 */
		function attachUrlToUsr(){
			var authorityGridManager = $("#authorityList").ligerGetGridManager();
			var authorityToSave = authorityGridManager.getUpdated();
			if(authorityToSave.length >0){
				$.ligerDialog.confirm('存在已赋权权限数据，是否清空并重新加载？', function (yes){
					if(yes){//
						reloadAuthority();
					}
				});
		...
		}//end-of attachUrlToUsr
		
		
		/**
		 * 重新加载权限数据
		 */
		function reloadAuthority(){
			var rows = 	urlList.getSelectedRows();
			var authorityGridManager = $("#authorityList").ligerGetGridManager();
			var usrGridManager =  $("#roleList").ligerGetGridManager();
			var usr = usrGridManager.getSelectedRow();
			if(!usr){
				$.ligerDialog.warn('请选择相应的用户');
				return;
			}
			//清空临时权限数据
			AuthorityUtil.manager.clearTempAuthority();
			for(var i = 0;i < rows.length;i++){
				AuthorityUtil.tempAuthority.push({name:rows[i].name,urlId:rows[i].id,usrId:usr.id});
			}
			authorityGridManager.reload({datas:AuthorityUtil.tempAuthority,Total:AuthorityUtil.tempAuthority.lenght});
		}//end-of reloadAuthority
